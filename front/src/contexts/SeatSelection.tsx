...
import { Armchair, Info, ArrowLeft } from 'lucide-react';
...
export function SeatSelection() {
  ...
  if (loading || !session) {
    return (
      <div className="min-h-screen bg-zinc-950 flex justify-center items-center">
        <div className="animate-spin text-yellow-400"><Armchair size={48} /></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 pb-20 font-sans selection:bg-yellow-400/30">
      {/* HEADER */}
      <motion.div 
      ...
      >
        <div className="w-full max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center">
          <div>
            <button onClick={() => navigate(-1)} className="text-zinc-400 hover:text-yellow-400 mb-3 flex items-center gap-2 text-sm font-bold tracking-wide transition-colors duration-300">
            ...
            <p className="text-zinc-400 mt-2 font-medium tracking-wide flex items-center gap-3">
              ...
              <span className="bg-yellow-400/20 text-yellow-400 border border-yellow-400/30 px-3 py-1 rounded-full text-xs font-bold">{session.detalhes.horario}</span>
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="col-span-2 flex flex-col items-center">
          ...
          <div className="w-full max-w-2xl h-24 relative flex items-end justify-center mb-24 mt-4 perspective-[1000px]">
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-yellow-400/5 to-transparent rounded-[100%]"></div>
                ...
                {cols.map((col) => {
                  ...
                  return (
                    <button
                      ...
                      className="relative flex flex-col items-center justify-center transition-all duration-300 hover:-translate-y-2 hover:scale-110 outline-none focus:ring-2 focus:ring-yellow-400/50 rounded-xl"
                    >
                      <Armchair 
                        ...
                        className={`w-8 h-8 lg:w-[42px] lg:h-[42px] transition-all duration-300 ${
                          ...
                          : isSelected ? 'text-yellow-400 drop-shadow-[0_0_12px_rgba(250,204,21,0.8)] fill-yellow-400/20 scale-110 -translate-y-1' 
                      ...
                      <span className={`text-[9px] font-black absolute bottom-[-14px] transition-colors duration-300 ${
                        isSelected ? 'text-yellow-400' : isOccupied ? 'text-transparent' : 'text-zinc-600 group-hover/row:text-zinc-400'
                      }`}>{col}</span>
                    </button>
                  );
                })}
          ...
          <div className="flex justify-center gap-10 mt-20 text-sm font-medium text-zinc-400 bg-zinc-900/60 backdrop-blur-md py-4 px-10 rounded-2xl border border-zinc-800/80 shadow-xl">
            ...
            <div className="flex items-center gap-3"><Armchair className="w-5 h-5 text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)] fill-yellow-400/20" /> Selecionado</div>
        ...
        <div className="bg-zinc-900/80 backdrop-blur-xl p-8 rounded-3xl border border-zinc-800/80 h-fit sticky top-32 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col transition-all duration-300 hover:border-zinc-700">
          <h3 className="text-2xl font-extrabold text-white mb-8 flex items-center gap-3"><Info className="text-yellow-400 w-6 h-6" /> Seu Ingresso</h3>
          ...
          <div className="flex-1 min-h-[120px]">
            {selectedSeats.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {selectedSeats.map(s => (
                  <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} key={s} className="bg-gradient-to-br from-zinc-800 to-zinc-900 px-4 py-2 rounded-xl text-yellow-400 font-black border border-zinc-700 shadow-inner tracking-wider">{s}</motion.span>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="border-t border-zinc-800 pt-6 mt-6">
            ...
            <div className="flex justify-between items-end mb-8">
              ...
              <span className="text-4xl font-black text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.2)]">R$ {totalValue.toFixed(2)}</span>
            </div>
            ...
            <button onClick={handleCheckout} disabled={selectedSeats.length === 0} className="w-full relative group/btn disabled:opacity-50 disabled:cursor-not-allowed">
              <div className="absolute inset-0 bg-yellow-400 rounded-2xl blur group-hover/btn:blur-md transition-all duration-300 opacity-70 group-disabled/btn:hidden"></div>
              <div className="relative bg-yellow-400 group-hover/btn:bg-yellow-400 text-zinc-950 font-black text-lg py-5 rounded-2xl transition-all duration-300 flex justify-center items-center gap-2 uppercase tracking-wide border border-yellow-400/50">
                Finalizar Compra
              </div>
            </button>
          </div>
